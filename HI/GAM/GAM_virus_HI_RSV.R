...
# Load data
data_infants <- read_excel("GAM_virus_HI_RSV_infants.xlsx")
data_mothers <- read_excel("GAM_virus_HI_RSV_mothers.xlsx")
...
# ggsave(file = "GAM_virus_infants_HI_RSV.png",
#        plot = plot1,
...
k_values2 <- 3 # Range of k values from 3 to 12. Had to increase and lower k
...
# Overlay data on smooths for the first plot
plot_obj2_with_data <- plot_obj2_with_data + geom_point(data = data_mothers, aes(x = days, y = Ct), size = 8,alpha = 0.7,shape=23, fill= "#3399CC",color = "#3399CC")
...
# Save the plot as a PNG file
# ggsave(file = "GAM_virus_mothers_HI_RSV.png",
#        plot = plot2,
#        width = 8,  # Width in inches
#        height = 6,  # Height in inches
#        units = "in",  # Specify units as inches

...
# Load data
data_infants <- read_excel("GAM_virus_RSV_HI_infants.xlsx")
data_mothers <- read_excel("GAM_virus_RSV_HI_mothers.xlsx")
...
k_values <- seq(3, 8)  # Range of k values from 3 to 12. Had to increase and lower k
...
plot_obj1_with_data <- plot_obj1_with_data + geom_point(data = data_infants, aes(x = days, y = Ct), size = 8,shape=23,fill="black",alpha = 0.6)
...
# ggsave(file = "GAM_virus_infants_RSV_HI_v2.png",
#        plot = plot1,
...
# There are only 3 data points in mothers
...
# Save the plot as a PNG file
# ggsave(file = "GAM_virus_mothers_RSV_HI.png",
#        plot = plot2,
#        width = 8,  # Width in inches
#        height = 6,  # Height in inches
#        units = "in",  # Specify units as inches

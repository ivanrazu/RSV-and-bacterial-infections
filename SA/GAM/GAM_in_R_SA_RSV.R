...
k_values <- seq(3, 11)  # Range of k values from 3 to 12. Had to increase and lower k
...
gam1 <- gam(Ct ~ s(days, bs = "cr", k=5), data = data_infants)
...
plot_obj1 <- plot(ggeffects::ggpredict(gam1), facets = TRUE) +
  ...
  labs(x = "Infant age (days)", y = "SA Ct") +

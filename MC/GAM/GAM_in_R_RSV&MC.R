...
# Load data
data_infants <- read_excel("GAM_RSV&MC_Infants.xlsx")
data_mothers <- read_excel("GAM_RSV&MC_Mothers.xlsx")
...
# ggsave(file = "GAM_infants_RSV&MC.png",
#        plot = plot1,
...
gam2a <- gam(Ct ~ s(days, bs = "cr"),  data = data_mothers )
gam2b <- gam(Ct ~ s(days, bs = "cr",k=3),  data = data_mothers )

AIC(gam2a,gam2b)

# Define the second plot
gam2 <- gam(Ct ~ s(days, bs = "cr"),  data = data_mothers )
plot_obj2 <- plot(ggeffects::ggpredict(gam2b), facets = TRUE) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high), fill = "white", alpha = 1) +
  coord_cartesian(xlim = c(0, 125), ylim = c(45, 20)) +  # Set the y-axis limits in reverse order
  labs(x = "Days after RSV+", y = "MC Ct") +
  scale_x_continuous(breaks = c(0, 30, 60, 90, 120)) +  # Set the x-axis ticks
...
# # Save the plot as a PNG file
# ggsave(file = "GAM_mothers_RSV&MC.png",
#        plot = plot2,
#        width = 8,  # Width in inches
#        height = 6,  # Height in inches
#        units = "in",  # Specify units as inches
